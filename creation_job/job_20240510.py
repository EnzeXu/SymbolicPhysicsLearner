...
tmux has-session -t {0}_default_{1}_{2} 2>/dev/null

if [ $? != 0 ]; then
  tmux new-session -d -s {0}_default_{1}_{2}

  tmux send-keys -t {0}_default_{1}_{2} "source ../Invariant_Physics/venv/bin/activate" C-m
  tmux send-keys -t {0}_default_{1}_{2} "bash jobs/job_20240510_{0}_default_{1}_{2}.sh" C-m
  echo "Launched jobs/job_20240510_{0}_default_{1}_{2}.sh on tmux session {0}_default_{1}_{2}"
  # If you want to leave the session detached, remove the line below
  # tmux send-keys -t {0}"exit" C-m
else
  echo "Session '{0}_default_{1}_{2}' already exists. Attaching..."
  tmux attach -t {0}_default_{1}_{2}
  tmux send-keys -t {0}_default_{1}_{2} "source ../Invariant_Physics/venv/bin/activate" C-m
  tmux send-keys -t {0}_default_{1}_{2} "bash jobs/job_20240401_{0}_default_{1}_{2}.sh" C-m
fi
"""
...
{0}
"""
noise_list = ["0.00", "0.05", "0.10", "0.15", "0.20"]

process_list_map = [[] for _ in range(len(noise_list))]
for k in range(0, 1, 1):
    # size = ["500", "1000"][k]
    for short in ["pp", "lorenz", "sir"]:

        for i, j in zip(range(len(noise_list)), noise_list):
            with open(f"jobs/job_20240510_{short}_default_{k}_{i+1}.sh", "w") as f:
                f.write(template.format(short_dic[short], k, j, eq_id_dic[short]))
            with open(f"launch_job/process_20240510_{short}_default_{k}_{i+1}.sh", "w") as f:
                f.write(launch_template.format(short, k, i+1))
            process_list_map[i].append(f"launch_job/process_20240510_{short}_default_{k}_{i+1}.sh")

for i, j in zip(range(len(noise_list)), noise_list):
    with open(f"launch_job/launch_20240510_server_{i+1}.sh", "w") as f:
        print(f"In launch_job/launch_20240510_server_{i+1}.sh")
        all_process = "\n".join([
