...
if [ $? != 0 ]; then
  ...
  tmux send-keys -t {0}_default_{1}_{2} "bash jobs/job_20240504_{0}_default_{1}_{2}.sh" C-m
  echo "Launched jobs/job_20240504_{0}_default_{1}_{2}.sh on tmux session {0}_default_{1}_{2}"
...
for k in range(10, 14, 1):
    ...
    for short in ["pp", "lorenz", "sir"]:
        ...
        for i, j in zip(range(len(noise_list)), noise_list):
            with open(f"jobs/job_20240504_{short}_default_{k}_{i+1}.sh", "w") as f:
                f.write(template.format(short_dic[short], k, j, eq_id_dic[short]))
            with open(f"launch_job/process_20240504_{short}_default_{k}_{i+1}.sh", "w") as f:
                f.write(launch_template.format(short, k, i+1))
            process_list_map[i].append(f"launch_job/process_20240504_{short}_default_{k}_{i+1}.sh")

for i, j in zip(range(len(noise_list)), noise_list):
    with open(f"launch_job/launch_20240504_server_{i+1}.sh", "w") as f:
        print(f"In launch_job/launch_20240504_server_{i+1}.sh")
        all_process = "\n".join([
