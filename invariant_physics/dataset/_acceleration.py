...
PARAMS_CONFIG = {
    ...
    "rule_map": ['A->(A+A)', 'A->(A-A)', 'A->(A*A)', 'A->(A/A)', 'A->(A*C)', 
                'A->x', 'A->y', 'A->z', 'A->C'],
    "ntn_list": ['A'],
    "truth_ode_format": ["x/z-{0}*y/z-{0}*9.80"],
    "purification_threshold": 0.03,
}


class Acceleration(ODEDataset):
    params_config = PARAMS_CONFIG
    
    def __init__(self, args):
        super(Acceleration, self).__init__(args, PARAMS_CONFIG, non_ode_function=True)
